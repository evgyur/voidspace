...
fn place_row(row: &[(usize, f64)], bounds: Rect, output: &mut [Rect]) -> Rect {
    ...
    if bounds.width() < bounds.height() {
        let row_height = ((sum / f64::from(bounds.width())).max(0.0) as f32).min(bounds.height());
        ...
        for (position, (index, area)) in row.iter().enumerate() {
            let width = if position + 1 == row.len() {
                ...
                ((*area / f64::from(row_height.max(f32::EPSILON))) as f32)
                    .min((bounds.max_x - x).max(0.0))
            }
            .max(0.0);
            let max_x = (x + width).min(bounds.max_x);
            output[*index] = Rect::new(
                x.min(bounds.max_x),
                bounds.min_y,
                max_x,
        ...
        let column_width = ((sum / f64::from(bounds.height())).max(0.0) as f32).min(bounds.width());
        ...
        for (position, (index, area)) in row.iter().enumerate() {
            let height = if position + 1 == row.len() {
                ...
                ((*area / f64::from(column_width.max(f32::EPSILON))) as f32)
                    .min((bounds.max_y - y).max(0.0))
            }
            .max(0.0);
            let max_y = (y + height).min(bounds.max_y);
            output[*index] = Rect::new(
                bounds.min_x,
                y.min(bounds.max_y),
