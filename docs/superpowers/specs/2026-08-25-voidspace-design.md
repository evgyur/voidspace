...
Если один watcher/fileop event пересекает две активные disjoint transactions (например, move из ветки A в B), coordinator не помещает его в два независимых buffers и не разрешает ни одному commit. Обе shadow arenas отменяются до publication; их buffers и crossing event сливаются по `(producer, sequence)` без reducer-применения, scope повышается до LCA(A,B), и запускается одна новая branch transaction с fence, равным самому раннему fence отменённых transactions. Watcher удерживает все события нового LCA после этого fence до replay. Если earliest cursor уже недоступен или LCA равен scope root при недостоверном buffer, применяется full-rescan rule. Поэтому один `(producer, sequence)` принадлежит ровно одному commit transaction.
...
Resource limits по умолчанию: файл ≤4 GiB, uncompressed ≤8 GiB, ≤50M nodes, каждый `WinName` ≤32 767 UTF-16 code units, каждая пользовательская UTF-8 string ≤32 KiB, nesting ≤256, compression ratio ≤100:1. Header и checksum проверяются до публикации; malformed, duplicate IDs, cycles, overflow агрегатов и превышение лимитов отклоняют весь snapshot. Никакой path из snapshot не используется для file operation; snapshot остаётся read-only.
