...
- Layout eligibility must include a minimum outer label footprint, not area alone. Before layout, the app compact-formats every real child that can enter the kept prefix and every saturating suffix sum that can become `OTHER` as that prefix is shortened, including the budget-limited suffix. It measures all those size-only galleys at the active DPI and uses the maximum measured width; measuring only individual children and the total is forbidden because formatted width is not monotonic across unit boundaries. It then adds tile padding, both visual insets, and stroke allowance before passing the footprint to `voidspace-layout`. Preview header space is removed from nested-layout bounds before this calculation is applied. A rectangle that satisfies the layout footprint must therefore still satisfy every possible real or aggregate size label for that candidate set after clipping and insets.
...
- Size label tier selection uses measured post-inset bounds and covers large, compact, size-only, aggregate, and minimum-canvas overflow cases. A regression case crosses a compact-unit boundary where an intermediate suffix such as `1023M` is wider than both its member labels and the final total such as `1.7G`.
